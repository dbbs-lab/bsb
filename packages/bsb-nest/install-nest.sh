...
if [ -z "$NEST_FOLDER" ]; then
  ...
  NEST_FOLDER="$(dirname "$(dirname $SCRIPT_DIR)")/.nx/installation/_nest_repo";
fi
# Get NEST version
if [ -z "$NEST_VERSION" ]; then NEST_VERSION="3.7"; fi

# Check if NEST has been already installed
if [ "$(uv run which nest)" != "" ];  then
  INSTALLED_VERSION=$(echo $(uv run nest --version) | grep -o -E 'version [0-9.]+' | sed 's/version //')
