...
_inner = bsb_otel.get_bsb_tracer("bsb-hdf5")


class _LocalHdf5Tracer:
    """
    Tracer wrapper that always runs spans inside ``bsb_otel.local_tracing``.
    ...
    ranks diverge. Forcing :func:`bsb_otel.local_tracing` keeps each rank's
    bsb-hdf5 spans local while still inheriting any pre-existing broadcast
    parent set up higher in the call stack.
    """
