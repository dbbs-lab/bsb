...
should yield the spike trains generated by :guilabel:`background noise`, as well as by
:guilabel:`base type` and :guilabel:`top type` cells.

A device writes one :class:`SpikeTrain <neo.core.SpikeTrain>` per cell it watched, empty ones
included, so its trains are its cells: there is no separate population size or list of spiking
IDs to read. Each train is annotated with the device that recorded it and the cell it belongs to,
accessible through the ``annotations`` attribute:
