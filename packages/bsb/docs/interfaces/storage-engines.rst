...
The bundle is what a simulation result file points back to: ``storage_id`` names the
reconstruction a recording was produced from, and ``state_id`` says which revision of
it, so a result can be traced to the exact network state that produced it.
...
bundle and emits a single
:class:`BsbProvenanceUpgradeWarning
<bsb:bsb.exceptions.BsbProvenanceUpgradeWarning>`.

.. warning::

    The backfill is **irreversible, and happens on open**. Merely opening a
    pre-provenance storage for writing rewrites it: the FS engine writes
    ``metadata.json`` and deletes the legacy ``versions.txt``, so BSB 7.x can no
    longer read that storage afterwards. There is no opt-out and no backup. Copy
    anything you still need to open with an older BSB before opening it with this
    one.

Read-only opens skip the backfill;
